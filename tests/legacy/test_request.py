...
class TestIncident(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_connection["host"] = (
            "%s.service-now.com" % self.mock_connection["instance"]
        )
    ...
    def test_connection(self):
        ...
        self.assertEqual(
            self.client.raise_on_empty, self.mock_connection["raise_on_empty"]
        )
    ...
    def test_get_linked_result(self):
        ...
        json_body_first = json.dumps(
            {"result": [{"number": self.mock_incident["number"], "linked": False}]}
        )
        json_body_second = json.dumps(
            {"result": [{"number": self.mock_incident["number"], "linked": True}]}
        )
    ...
    def test_insert_incident(self):
        ...
        result = self.client.insert(
            table="incident", payload={"field1": "value1", "field2": "value2"}
        )
    ...
    def test_insert_incident_invalid_status(self):
        ...
        """
        json_body = {
            "result": [{"sys_id": self.mock_incident["sys_id"]}],
            "error": {"message": "Error summary", "detail": "Error detail "},
        }
